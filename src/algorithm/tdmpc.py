...
class TOLD(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self._encoder = ag.RepresentationAgent(cfg)
        self._dynamics = ag.LatentDynamicAgent(cfg)
        self._reward = ag.RewardAgent(cfg)
        self._pi = ag.PolicyAgent(cfg)
        self._Q = ag.ValueAgent(cfg)
        self.apply(h.orthogonal_init)
        for m in [self._reward._reward, self._Q._Q1, self._Q._Q2]:
            m[-1].weight.data.fill_(0)
            m[-1].bias.data.fill_(0)

    def track_q_grad(self, enable=True):
        """Utility function. Enables/disables gradient tracking of Q-networks."""
        for m in [self._Q._Q1, self._Q._Q2]:
    ...
    def next(self, z, a):
        ...
        return self._dynamics(x=x), self._reward(x=x)

    def pi(self, z, std=0):
        """Samples an action from the learned policy (pi)."""
        mu = torch.tanh(self._pi(x=z))
        if std > 0:
