...
class ReplayBuffer():
	...
	def __add__(self, episode: Workspace):
		self.add(episode)
		return self

	def add(self, episode: Workspace):
		obs_raw = episode.get_full('env/env_obs').to(self.device)
		action_raw = episode.get_full('action').to(self.device)
		reward_raw = episode.get_full('env/reward').to(self.device)

		obs = obs_raw[0]
		last_obs = obs_raw[1][-1]
		action = action_raw[0]
		# last_action = action_raw[1][-1].view(1, -1)
		reward = reward_raw[0]
		# last_reward = reward_raw[1][-1].view(1, -1)

		self._obs[self.idx:self.idx+self.cfg.episode_length] = obs[:-1] if self.cfg.modality == 'state' else obs[:, -3:]
		self._last_obs[self.idx//self.cfg.episode_length] = last_obs
		self._action[self.idx:self.idx+self.cfg.episode_length] = action
		self._reward[self.idx:self.idx+self.cfg.episode_length] = reward
		if self._full:
	...
	def _get_obs(self, arr, idxs):
		if self.cfg.modality == 'state':
			return arr[idxs]
		obs = torch.empty((self.cfg.batch_size, 3*self.cfg.frame_stack, *arr.shape[-2:]), dtype=arr.dtype, device=self.device)
...
def estimate_value(model, cfg, z, actions, horizon):
	"""Estimate value of a trajectory starting at latent state z and executing given actions."""
	G, discount = 0, 1
	for t in range(horizon):
		z, reward = model.next(z, actions[t])
		G += discount * reward
		discount *= cfg.discount
	G += discount * torch.min(*model.Q(z, model.pi(z, cfg.min_std)))
	return G