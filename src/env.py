...
class ActionRepeatWrapper(dm_env.Environment):
	...
	def step(self, action):
		...
		return time_step._replace(reward=reward, discount=discount) # {} # Add info for gymnasium compatibility
...
class ExtendedTimeStepWrapper(dm_env.Environment):
	...
	# Added for dm_control compatibility
	def _replace(self, observation=None):
		return self.env._replace(observation=observation)
...
class TimeStepToGymWrapper(object):
	...
	# seed and options are useless... I guess
	def reset(self, seed=None, options=None):
		self.t = 0
		res = self.env.reset()
		# I supposed that info is useless
		return self._obs_to_array(res.observation), {} # , False, False, res.reward
...
class AutoResetAdapterWrapper(AutoResetWrapper):
	def __init__(self, env):
		super().__init__(env)
...
def make_env(cfg, autoreset):
	...
	if autoreset:
		env = AutoResetAdapterWrapper(env)
